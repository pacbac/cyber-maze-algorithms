...
def correctMinPos(intersectionArr, type):
    print "intersectionArr: " + str(intersectionArr)
    if type == 0:
...
def control_robot(robot): #main method, robot object
    ...
    while virusesLeft > 0:
        ...
        if virusesLeft == 1 and packetsExist:
            ...
            if curPos in viruses_list: #sometimes viruses_list has an element that is already equal to curPos. just a safety measure to remove it
                viruses_list.remove(curPos)
                
            while not (virusesLeft == 1 and packetsExist):
                ...
                if isIntersection: #if it's an intersection             
                    ...
                    if curPos not in intersection: #if this is a new intersection, then we need to add in data
                        ...
                        if curPos == prevPos:
                            ...
                            robot.turn_right()
                            if robot.sense_steps(robot.SENSOR_RIGHT) > 0: #check if going backwards is valid for starting point (0, 0) when it's an intersection
                        ...
                        if previousNode not in intersection[curPos]: #if previous node is not in open area, set previous node as safe intersection to backtrack to if there's trouble
                            openSpacePtr = [[], []] #aka if previousNode is not adjacent to current intersection (then it's not a 2x2 or more open area)
                            backtrack = previousNode #[[intersections], [pointers]]
                ...
                if curPos not in intersection:
                    ...
                    #force the robot to not follow the traditional right/left/straight hand rule so it can break out of the loop
